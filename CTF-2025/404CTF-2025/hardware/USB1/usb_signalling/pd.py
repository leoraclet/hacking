...
class Decoder(srd.Decoder):
    ...
    options = (
        {
            ...
            "desc": "Signalling",
            "default": "automatic",
            "values": ("automatic", "full-speed", "low-speed"),
        },
    )
    annotations = (
        ("sym-j", "J symbol"),
    ...
    def get_bit(self, sym):
        ...
        if len(self.bits) <= 24:
            self.bits += b
        if len(self.bits) == 24 and self.bits == "000000000000000100111100":
    ...
    def decode(self):
        ...
        while True:
            ...
            if self.state == St.IDLE:
                ...
                self.edgepins = self.wait(
                    [{"skip": self.samplenum_edge - self.samplenum}]
                )
