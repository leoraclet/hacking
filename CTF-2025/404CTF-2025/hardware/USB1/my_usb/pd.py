...
pids = {
    ...
    "10110100": [
        "SETUP",
        "Address & EP number in host-to-function transaction for SETUP to a control pipe",
    ],
    # Data
    ...
    "11100001": [
        "DATA2",
        "Data packet PID HS, high bandwidth isosynchronous transaction in a microframe",
    ],
    "11110000": [
        "MDATA",
        "Data packet PID HS for split and high-bandwidth isosynchronous transactions",
    ],
    # Handshake
    ...
    "00111100": [
        "PRE",
        "Host-issued preamble; enables downstream bus traffic to low-speed devices",
    ],
...
class Decoder(srd.Decoder):
    ...
    desc = "USB (low-speed and full-speed) packet protocol. Hacked !"
    license = "gplv2+"
    ...
    def handle_packet(self):
        ...
        # Bits[0:7]: SYNC
        sync = packet[: 7 + 1 + OFFSET]
        self.ss, self.es = self.bits[0][1], self.bits[7 + OFFSET][2]
        # The SYNC pattern for low-speed/full-speed is KJKJKJKK (00000001).
        if sync != "0000000000000001":
        ...
        # Bits[8:15]: PID
        pid = packet[8 + OFFSET : 15 + 1 + OFFSET]
        pidname = pids.get(pid, ("UNKNOWN", "Unknown PID"))[0]
        self.ss, self.es = self.bits[8 + OFFSET][1], self.bits[15 + OFFSET][2]
        ...
        if pidname in ("OUT", "IN", "SOF", "SETUP", "PING"):
            ...
            if pidname == "SOF":
                # Bits[16:26]: Framenum
                framenum = bitstr_to_num(packet[16 + OFFSET : 26 + 1 + OFFSET])
                self.ss, self.es = self.bits[16 + OFFSET][1], self.bits[26 + OFFSET][2]
                ...
                # Bits[16:22]: Addr
                addr = bitstr_to_num(packet[16 + OFFSET : 22 + 1 + OFFSET])
                self.ss, self.es = self.bits[16 + OFFSET][1], self.bits[22 + OFFSET][2]
                self.putpb(["ADDR", addr])
                self.putb([4, ["Address: %d" % addr, "Addr: %d" % addr, "Addr", "A"]])
                self.packet.append(addr)
                self.packet_summary += " ADDR %d" % addr

                # Bits[23:26]: EP
                ep = bitstr_to_num(packet[23 + OFFSET : 26 + 1 + OFFSET])
                self.ss, self.es = self.bits[23 + OFFSET][1], self.bits[26 + OFFSET][2]
                self.putpb(["EP", ep])
            ...
            # Bits[27:31]: CRC5
            crc5 = bitstr_to_num(packet[27 + OFFSET : 31 + 1 + OFFSET])
            crc5_calc = calc_crc5(packet[16 + OFFSET : 27 + OFFSET])
            self.ss, self.es = self.bits[27 + OFFSET][1], self.bits[31 + OFFSET][2]
            if crc5 == crc5_calc:
            ...
            for i in range(0, len(data), 8):
                ...
                self.ss, self.es = (
                    self.bits[16 + i + OFFSET][1],
                    self.bits[23 + i + OFFSET][2],
                )
